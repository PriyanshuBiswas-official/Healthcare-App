...
export default function AppointmentsRemindersScreen({ onBack, onSaved }: Props) {
  ...
  const addEntry = () => {
    ...
    setEntries(prev => [
      ...prev,
      { doctor: newDoctor.trim(), specialty: newSpecialty.trim(), dateTime: newDateTime.trim(), location: newLocation.trim() },
    ]);
    setNewDoctor('');
    setNewSpecialty('');
    setNewDateTime('');
  ...
  const handleSave = () => {
    setEditing(false);
    onSaved?.();
    Alert.alert('Saved', 'Appointment reminders updated');
  };
          ...
          <>
                  ...
                  <View key={i}>
                    <View style={styles.entryRow}>
                      <View style={styles.entryInfo}>
                        <Text style={styles.entryName}>{entry.doctor}</Text>
                        <Text style={styles.entryDetail}>
                          {entry.specialty}{entry.dateTime ? ` · ${entry.dateTime}` : ''}
                        </Text>
                        {entry.location ? (
            ...
            <GlassCardView style={styles.card}>
              ...
              <TextInput
                ...
                placeholder="Date & time (e.g. Jun 20, 10:30 AM)"
                placeholderTextColor={Colors.textMuted}
              />
              <TextInput
                style={styles.input}
