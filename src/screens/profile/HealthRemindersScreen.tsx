...
export default function HealthRemindersScreen({ onBack, onSaved }: Props) {
  ...
  const addEntry = () => {
    ...
    setEntries(prev => [
      ...prev,
      { title: newTitle.trim(), description: newDescription.trim(), frequency: newFrequency.trim(), time: newTime.trim() },
    ]);
    setNewTitle('');
    setNewDescription('');
    setNewFrequency('');
  ...
  const handleSave = () => {
    setEditing(false);
    onSaved?.();
    Alert.alert('Saved', 'Health reminders updated');
  };
                  ...
                  <View key={i}>
                    <View style={styles.entryRow}>
                      <View style={styles.entryInfo}>
                        <Text style={styles.entryName}>{entry.title}</Text>
                        <Text style={styles.entryDetail}>
                          {entry.description}{entry.frequency ? ` · ${entry.frequency}` : ''}{entry.time ? ` · ${entry.time}` : ''}
                        </Text>
                      </View>
